...
# Langfuse 是可选依赖，这里做一下兼容处理，避免没装或版本不匹配时直接报错
try:
...
class TraceLangfuseDecorator:
    ...
    def init_langfuse_handler(self):
        # 只有在同时配置了 Langfuse 环境变量且模块存在时才启用
        if (
            LlamaIndexCallbackHandler is not None
            and os.environ.get('LANGFUSE_SECRET_KEY') is not None
        ):
            ...
            print('Warning: Langfuse not configured or not installed. Skipping Langfuse callback handler.')
