...
打开docker运行
docker compose up -d
然后在系统环境变量中设置：（终端，不用修改python）
set OPENAI_API_KEY='sk-FGHIXlyPYpUGzovjKzG7UYv7J7vfJYevqKsEf8o3EryiuiCA'
set NEO4J_USERNAME=neo4j
set NEO4J_PASSWORD=cmdbench
set PGUSER=megagon
（连接失败的时候：手动开一个neoj的docker）
'''
...
def test_mongo():
    ...
    assert len(docs) == 1 and docs[0]['wikipedia_title'] == 'LeBron James'   # 注释掉就成功了www
