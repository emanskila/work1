...
# 找到每个聚类中与质心最接近的原始标题，作为代表样本
def get_centroid_titles(titles: List[str], model: str, n_clusters: int = 20):
    model = SentenceTransformer(model)  
    embeddings = model.encode(titles, show_progress_bar=True)  # 生成标题嵌入
    embeddings = np.array(embeddings)  # (n_samples, n_features)
    kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(embeddings)  # 聚类
    ...
    cosine_sim = (embeddings @ centroids.T)  # (n_samples, n_clusters) # 计算余弦相似度
    most_similar_ids = np.argmax(cosine_sim, axis=0) # 找到最相似的簇
