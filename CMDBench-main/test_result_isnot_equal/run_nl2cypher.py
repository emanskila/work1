...
class KnowledgeGraphQueryEngineWithProvenance(KnowledgeGraphQueryEngine):
    ...
    def _get_evidence_subgraph(self, cypher):  # 基于查询执行，收集证据子图
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        try:
            nodes = super()._retrieve(query_bundle)  # 拿到父类记录的nodes
...
def get_graph_query_engine(llm, verbose=False):
    if llm.startswith("gpt"):
        Settings.llm = OpenAI(temperature=0, model=llm)   # 配置Setting
    else:
...
def get_all_responses(engine, dataset) -> List[dict]:
    ...
    for d in dataset:
        d = copy.deepcopy(d)
        response = engine.query(d["question"])  # 基于LLM生成查询并执行，返回结果
...
def main():
    ...
    parser.add_argument('--inputs', default=["benchmark/q_graph_single1.json"], nargs="+")
    ...
    parser.add_argument('--llm', default="gpt-4-turbo-preview")   # LLM选取GPT-4
    ...
    if args.print_graph_schema:
        graph_schema = engine.graph_store.get_schema()  # 获取图数据库的Schema
        print(graph_schema)
        return

    if args.question:
        engine.query(args.question)  # 执行一次查询
        return
